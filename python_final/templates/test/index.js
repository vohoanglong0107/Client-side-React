...
const Home = () => {
	...
	const Post = (props) => {
		...
		return (
			<div class="post-container">
				<div class="ava-name">
					<img src="../assets/3135715.png" alt="user-avatar" class="user-avatar" />
					<p class="username">{props.username}</p>
				</div>
				<p class="content"><strong>{props.content}</strong></p>
				<hr class="hr" />
				<div class="react">
					<button class={ !react ? 'react-button heart' : 'react-button hearted'}  onClick={() => {
						setReact(!react)
				...
				{visible? (
					...
					<img src="../assets/3135715.png" alt="avatar" class="comment-avatar"/>
					<input placeholder="write comment..." class="comment-input" onChange={(e) => setNewComment(e.target.value)} onKeyDown={(event ) => {handleEnter(event,props.postID)}} />
					
					{showCmts ? (
						<div class="list-cmts">
						{listComments.map((item) => {
							return (
							<Comment comment={item.comment} avatar={item.avatar} username={item.username} />
							)
						})}
						</div>
					)
						: <label class="show-all-cmts" onClick={() =>{
							setShowCmts(true); 
							setCount((prevCount) => {
	...
	return (
		<div class="container">
			...
			<main>
				<div class="news">
					<img src="../assets/3135715.png" alt="user-icon" class="user-avatar"/> 
					<input class="news-text" value={input} placeholder="What are you thinking?" onChange={(e) => setInput(e.target.value)}/>
					<br />
					<button class="news-button" onClick={() => {handleAddPost()}} >Post</button>
				</div>
				<div>
				{datas.map((item) => {
					return (
					<Post username={item.username} postID={item.postID} commentText={item.commentText} content={item.content} loves={item.loves} comments={item.comments}/>
					);
				})}
				</div>
					...
					<form class="inputs-form">
						<div class="inputs-field container">
							<div class="input-div row">
								...
								<input class="inputs col-8" type="text" placeholder="your email..." onChange={(e) => setInputs({...inputs, email: e.target.value})} />
							</div>
							<div class="input-div row">
								<label class="labels col-4">Username:</label>
								<input class="inputs col-8" type="text" placeholder="your username..." onChange={(e) => setInputs({...inputs, username: e.target.value})} />
							</div>
							<div class="input-div row">
								<label class="labels col-4">Password:</label>
								<input class="inputs col-8" type="password" placeholder="your password..." onChange={(e) => setInputs({...inputs, password: e.target.value})}/>
							</div>
						</div>
						<div>
