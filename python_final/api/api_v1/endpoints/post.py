...
def route_create_posts(body):
    ...
    user = create_post(db_session, body, current_user.id)
    return user
...
@docs.register
@doc(description="Get post by user id", security=security_params, tags=["posts"])
...
def route_update_posts(id):
    ...
    user = get_post_by_user_id(db_session, id)
    return user
